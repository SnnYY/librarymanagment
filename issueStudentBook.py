...
def issue(student_number):
    ...
    updateStatus = "update "+bookTable+" set status = 'Rented' where bid = '"+bid+"'"
    try:
        if status == True:
...
def issueStudentBook(student_number): 
    ...
    #Issue Button
    issueBtn = Button(root,text="Issue",bg='#d1ccc0', fg='black',command=lambda: issue(student_number))
